...
class KickedRotor1D:
    def __init__(
        self, kicking_strength: float, initial_state: Optional[np.ndarray] = None
    ) -> None:
    ...
    def chirikov_map(
        self, phasepoint: np.ndarray, kicking_strength: float
    ) -> np.ndarray:
        """Returns the Chirikov map of the given phase-point"""

        new_phasepoint = np.zeros((phasepoint.shape))
        new_phasepoint[1] = np.mod(
            phasepoint[1] + kicking_strength * np.sin(phasepoint[0]), 2 * np.pi
        )
    ...
    def get_phasespace(self, ntimesteps: int) -> np.ndarray:
        ...
        for i in range(len(phasespace[0]) - 1):
            phasespace[:, i + 1] = self.chirikov_map(
                phasespace[:, i], self.kicking_strength
            )
        return phasespace
